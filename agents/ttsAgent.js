 ...
 * @param {{ larvoiceKeys?: string[], larvoiceKey?: string, larvoiceVoiceId?: string|number, outputLanguage?: string, sessionId?: string, useLarVoice?: boolean, edgeTTSVoice?: string }} [keys]
 */
...
// ─── Edge TTS (Microsoft Edge's free TTS) ───────────────────────────────────
...
async function generateTTS_EdgeTTS(text, outputPath, onLog, keys = {}) {
  ...
  const defaultVoice = language === 'en' 
    ? 'en-US-AriaNeural'  // Giọng nữ tự nhiên cho tiếng Anh
    : 'vi-VN-HoaiMyNeural'; // Giọng nữ tự nhiên cho tiếng Việt
  
  const voice = keys.edgeTTSVoice || defaultVoice;
  ...
  try {
    // Tạo file tạm để edge-tts ghi vào
    const tempOutput = outputPath.replace(/\.mp3$/, '.temp.mp3');
    ...
    // Chuyển đổi sang MP3 với ffmpeg nếu cần
    if (fs.existsSync(tempOutput)) {
      await new Promise((resolve, reject) => {
        ...
        p.on('close', code => {
          fs.unlinkSync(tempOutput); // Xóa file tạm
          if (code === 0) resolve();
    ...
    onLog?.(`TTS Edge: Lỗi: ${err.message}`);
...
async function generateTTS_LarVoice(text, outputPath, onLog, keys = {}) {
  ...
  const isMock = process.env.MOCK_TTS === 'true' || keyList.includes('mock') || keyList.includes('dummy');

  if (!keyList.length && !isMock) {
    throw new Error('Thiếu LarVoice API key — nhập key trên giao diện');
  }

  if (isMock) {
    onLog?.(`[TTS Mock] Generating mock silent TTS for text: "${text.slice(0, 40)}..."`);
    const wordCount = text.split(/\s+/).filter(Boolean).length;
    const duration = Math.max(3, Math.min(15, Math.ceil(wordCount * 0.35)));
    
    await new Promise((resolve, reject) => {
      const p = spawn(FFMPEG_BIN, [
        '-y', '-f', 'lavfi', '-i', 'anullsrc=r=44100:cl=mono',
        '-t', String(duration), '-q:a', '9', outputPath
      ]);
      p.on('close', code => {
        if (code === 0) resolve();
        else reject(new Error(`FFmpeg failed to generate mock TTS with code ${code}`));
      });
    });
    
    return { duration, uuid: 'mock-uuid', subtitleUrl: null };
  }
