...
export function getHyperframesReviewScene(i, scene, sceneId, start) {
  ...
  switch (i) {
    ...
    case 4: {
      ...
      html = `
        ...
        <div class="repo-check-panel signal-check-list" style="position:absolute; top:515px; left:50%; transform:translateX(-50%); width:920px; z-index:30;" id="check-${sceneId}">
          ${checkCards.slice(0, 4).map((card, idx) => `
          <div class="signal-check-row bento-card" id="check-${sceneId}-${idx + 1}">
            <div class="signal-check-index">${String(idx + 1).padStart(2, "0")}</div>
            <div class="signal-check-meter"><span style="width:${52 + idx * 12}%"></span></div>
            <div class="signal-check-copy">
              <div class="check-title card-title">${card.title}</div>
              <div class="check-desc card-desc">${card.body}</div>
            </div>
          </div>`).join("")}
        </div>
      `;
  ...
  html = `<div class="knowledge-map-template knowledge-map-card">${html}</div>`;
  if (/class="(?:bento-card|step-card|web-card|terminal-frame)/.test(html)) {
    gsap += `\n        tl.from("#${sceneId} .knowledge-map-card .bento-card, #${sceneId} .knowledge-map-card .step-card, #${sceneId} .knowledge-map-card .web-card, #${sceneId} .knowledge-map-card .terminal-frame", { x: -34, duration: 0.22, stagger: 0.05, ease: "power3.out" }, ${start + 0.14});`;
  }
  return { html, gsap };
}
