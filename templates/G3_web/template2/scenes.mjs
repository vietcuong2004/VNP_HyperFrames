...
export function getHyperframesReviewScene(i, scene, sceneId, start) {
  ...
  if (/class="(?:bento-card|step-card|web-card|terminal-frame)/.test(html)) {
    gsap += `\n        tl.from("#${sceneId} .source-note-card .bento-card, #${sceneId} .source-note-card .step-card, #${sceneId} .source-note-card .web-card, #${sceneId} .source-note-card .terminal-frame", { y: 24, opacity: 0, duration: 0.22, stagger: 0.05, ease: "power3.out" }, ${start + 0.14});`;
  }
  return { html, gsap };
}
