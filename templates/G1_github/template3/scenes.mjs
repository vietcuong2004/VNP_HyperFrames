...
export function getHyperframesReviewScene(i, scene, sceneId, start) {
  ...
  switch (i) {
    ...
    case 4: {
      ...
      html = `
        ...
        <div class="repo-check-panel brief-command-list" style="position:absolute; top:515px; left:50%; transform:translateX(-50%); width:920px; z-index:30;" id="check-${sceneId}">
          ${checkCards.slice(0, 4).map((card, idx) => `
          <div class="brief-check-command step-card" id="check-${sceneId}-${idx + 1}">
            <div class="step-number">${idx + 1}</div>
            <div class="brief-command-copy">
              <div class="check-title step-title"><span class="brief-prompt">$</span> ${card.title}</div>
              <div class="check-desc step-desc">${card.body}</div>
            </div>
            <div class="brief-command-status">READY</div>
          </div>`).join("")}
        </div>
      `;
      gsap = `
  ...
  if (/class="(?:bento-card|step-card|web-card|terminal-frame)/.test(html)) {
    gsap += `\n        tl.from("#${sceneId} .dev-brief-card .bento-card, #${sceneId} .dev-brief-card .step-card, #${sceneId} .dev-brief-card .web-card, #${sceneId} .dev-brief-card .terminal-frame", { y: -24, duration: 0.22, stagger: 0.05, ease: "power3.out" }, ${start + 0.14});`;
  }
  return { html, gsap };
}
