...
const SCENE_ORDER = [0, 3, 1, 5, 2, 6, 4, 7];
...
`;
      ...
      scenesHTML += `
      <div id="${sceneId}" class="scene variant-scene variant-${visualIndex + 1}" style="position:absolute; inset:0; opacity:0; visibility:hidden; z-index: 10;">
        ${sceneVisualHtml}
        ${charHtml}
      </div>`;
      ...
      tl.from("#${sceneId}", { clipPath: "inset(0 0 100% 0)", duration: 0.24, ease: "power3.out" }, ${start - 0.08});
      ${sceneGsap}
      `;
      if (charHtml) {
        const sceneDuration =
      ...
      tl.eventCallback("onUpdate", () => {
        ...
        if (activeBlock) {
          let html = activeBlock.words.map(w => {
            ...
            return '<span style="font-family: var(--font-sans); font-size: 34px; font-weight: 800; text-transform: uppercase; margin: 0 7px; display: inline-block; transition: all 0.08s; ' +
