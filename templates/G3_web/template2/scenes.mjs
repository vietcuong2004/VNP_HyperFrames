...
export function getHyperframesReviewScene(i, scene, sceneId, start) {
  ...
  switch (i) {
    ...
    case 3: {
      ...
      html = `
        ...
        <div class="web-evidence-panel source-note-stack" style="position:absolute; top:455px; left:50%; transform:translateX(-50%); width:920px; z-index:30;">
          ${evidenceCards.slice(0, 3).map((card, idx) => `
          <div class="source-note-row" id="ev-${sceneId}-${idx + 1}">
            <div class="source-note-tab">NOTE ${idx + 1}</div>
            <div class="source-note-copy">
              <div class="source-note-title">${card.title}</div>
              <div class="source-note-desc">${card.body}</div>
            </div>
          </div>`).join("")}
        </div>
      `;
  ...
  html = `<div class="docs-path-template source-note-card">${html}</div>`;
  if (/class="(?:bento-card|step-card|web-card|terminal-frame)/.test(html)) {
    gsap += `\n        tl.from("#${sceneId} .source-note-card .bento-card, #${sceneId} .source-note-card .step-card, #${sceneId} .source-note-card .web-card, #${sceneId} .source-note-card .terminal-frame", { y: 24, duration: 0.22, stagger: 0.05, ease: "power3.out" }, ${start + 0.14});`;
  }
  return { html, gsap };
}
