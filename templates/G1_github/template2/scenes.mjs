...
export function getHyperframesReviewScene(i, scene, sceneId, start) {
  ...
  if (/class="(?:bento-card|step-card|web-card|terminal-frame)/.test(html)) {
    gsap += `\n        tl.from("#${sceneId} .knowledge-map-card .bento-card, #${sceneId} .knowledge-map-card .step-card, #${sceneId} .knowledge-map-card .web-card, #${sceneId} .knowledge-map-card .terminal-frame", { x: -34, opacity: 0, duration: 0.22, stagger: 0.05, ease: "power3.out" }, ${start + 0.14});`;
  }
  return { html, gsap };
}
